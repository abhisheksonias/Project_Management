...
interface CreateTaskDialogProps {
  ...
  description?: string;
  submitLabel?: string;
  disableAssigneeSelection?: boolean;
}
...
export const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  disableAssigneeSelection = false,
}) => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            <div>
              ...
              {disableAssigneeSelection ? (
                <div className="rounded-[14px] border border-input bg-muted/40 px-3 py-2 text-sm text-muted-foreground">
                  {data.assigned_user_ids && data.assigned_user_ids.length > 0
                    ? `${data.assigned_user_ids.length} user selected (auto-assigned)`

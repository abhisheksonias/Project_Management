...
export const AdminTaskDetailsPanel: React.FC<AdminTaskDetailsPanelProps> = ({
  ...
  const uploadCommentImage = (file: File) =>
    uploadTaskCommentImage(file, activeTask.id, activeTask.project_id);

  const handleAddComment = () => {
    if (!hasCommentContent(commentText) || !profile || !activeTask) return;
  ...
  return (
    <>
      <Sheet open={open} onOpenChange={onClose}>
        <SheetContent className="flex h-full w-full flex-col sm:max-w-2xl p-3 pb-8 sm:p-4 sm:pb-10 md:p-6 md:pb-12">
          ...
          <Tabs
            ...
            className="mt-4 sm:mt-6 flex h-full min-h-0 flex-col"
          >
            ...
            <TabsContent
              value="details"
              className="mt-4 flex-1 min-h-0 overflow-y-auto pr-1 pb-14"
            >

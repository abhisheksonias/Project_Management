...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-card border border-border rounded-[14px] shadow-lg p-3 min-w-[180px]">
  ...
  return (
    <Card className="rounded-[14px] border-2 shadow-lg bg-card">
      <CardHeader className="pb-2 sm:pb-3">
        <CardTitle className="text-sm sm:text-base md:text-lg text-foreground font-semibold">Hours Logged frequency</CardTitle>
        ...
        <div className="h-[200px] sm:h-[240px] md:h-[280px] w-full">
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData || []} margin={{ top: 5, right: 8, bottom: 20, left: -12 }}>
